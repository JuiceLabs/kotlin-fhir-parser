...
class FhirSpec(private val directory: String) { // todo  add supprt for settings
    ...
    fun asClassName(className: String, parentName: String?): String? {
        ...
        if (pathName != null && Mappings.classMap.containsKey(pathName)) {
            return Mappings.classMap[pathName]
        }

        // is our plain class mapped?
        if (Mappings.classMap.containsKey(className)) {
            return Mappings.classMap[className]
        }
    ...
    fun classNameForTypeIfProperty(typeName: String): String? {
        ...
        return if (className != null) Mappings.reservedMap.getOrDefault(className, className) else null
    }
    ...
    fun classNameIsNative(className: String): Boolean {
        return Mappings.natives.contains(className)
    }


    fun safePropertyName(propName: String): String {
        return Mappings.reservedMap.getOrDefault(propName, propName)
    }


    // todo imp
    fun safeEnumName(name: String): String {
        return Mappings.enumNameMap.getOrDefault(name, name)

    }


    fun jsonClassForClassName(className: String): String {
        return Mappings.jsonmap.getOrDefault(className, Mappings.jsonMapDefault)
    }
